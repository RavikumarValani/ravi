...
class Ccc_Compile_Adminhtml_Compile_setController extends Mage_Adminhtml_Controller_Action
{

	protected function _setTypeId()
    {
        Mage::register('entityType',
            Mage::getModel('compile/compile')->getResource()->getTypeId());
    }
	...
	public function indexAction()
	{
        ...
        $this->_addBreadcrumb(Mage::helper('compile')->__('Compile'), Mage::helper('compile')->__('Compile'));
        $this->_addBreadcrumb(
            Mage::helper('compile')->__('Manage Compile Sets'),
            Mage::helper('compile')->__('Manage Compile Sets'));

        $this->_addContent($this->getLayout()->createBlock('compile/adminhtml_compile_attribute_set_toolbar_main'));
      
        $this->_addContent($this->getLayout()->createBlock('compile/adminhtml_compile_attribute_set_grid'));
	...
	public function addAction()
    {
        $this->_title($this->__('Compile'))
             ->_title($this->__('Attributes'))
             ->_title($this->__('Manage Compile Attribute Sets'))
        ...
        $this->_addContent($this->getLayout()->createBlock('compile/adminhtml_compile_attribute_set_toolbar_add'));
    ...
    public function editAction()
    {
        ...
        $this->_addBreadcrumb(Mage::helper('compile')->__('Compile'), Mage::helper('compile')->__('Compile'));
        $this->_addBreadcrumb(
            Mage::helper('compile')->__('Manage Compile Attribute Sets'),
            Mage::helper('compile')->__('Manage Compile Attribute Sets'));

        $this->_addContent($this->getLayout()->createBlock('compile/adminhtml_compile_attribute_set_main'));
    ...
    public function saveAction()
    {
        ...
        try {
            if ($isNewSet) {
                ...
                if (!$model->getId()) {
                    Mage::throwException(Mage::helper('compile')->__('This attribute set no longer exists.'));
                }
            ...
            $model->save();
            $this->_getSession()->addSuccess(Mage::helper('compile')->__('The attribute set has been saved.'));
        } catch (Mage_Core_Exception $e) {
            $this->_getSession()->addError($e->getMessage());
            $hasError = true;
        } catch (Exception $e) {
            $this->_getSession()->addException($e,
                Mage::helper('compile')->__('An error occurred while saving the attribute set.'));
            $hasError = true;
        }
